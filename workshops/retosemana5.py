...
# Propiedades de color de la terminal
class term_color:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKCYAN = '\033[96m'
...
def crud(name, lista):
    ...
    if opcion == 1:
        lista.append(input(f'{term_color.ENDC}Ingrese '+str(name[:-2])+': 'f'{term_color.OKGREEN}'))
        print(f'{term_color.OKBLUE}Se ha añadido un nuevo '+str(name[:-2])+f'!{term_color.ENDC}')
    if opcion == 2:
        print(str(lista[int(input(f'{term_color.ENDC}Ingrese id del '+str(name[:-2])+' a consultar: 'f'{term_color.OKCYAN}'))]))
    if opcion == 3:
        del lista[int(input(f'{term_color.ENDC}Ingrese id del '+str(name[:-2])+' a eliminar: 'f'{term_color.OKCYAN}'))]
        print(f'{term_color.OKBLUE}Se ha eliminado el '+str(name[:-2])+f'!{term_color.ENDC}')
    if opcion == 4:
        lista[int(input(f'{term_color.ENDC}Ingrese id del '+str(name[:-2])+' a editar: 'f'{term_color.OKCYAN}'))] = input(f'{term_color.ENDC}Ingrese nuevo '+str(name[:-2])+': 'f'{term_color.OKGREEN}')
        print(f'{term_color.OKBLUE}Se ha editado el '+str(name[:-2])+f'!{term_color.ENDC}')
    elif opcion == 5:
        i = 0
        while i <= len(lista)-1:
...
def crudVentas(name, lista):
    ...
    if opcion == 2:
        _id = int(input(f'{term_color.ENDC}Ingrese id de la venta a consultar: 'f'{term_color.OKCYAN}'))
        printVenta(lista[_id], _id)
    if opcion == 3:
        del lista[int(input(f'{term_color.ENDC}Ingrese id de la venta a eliminar: 'f'{term_color.OKCYAN}'))]
        print(f'{term_color.OKBLUE}Se ha eliminado la venta!{term_color.ENDC}')
    if opcion == 4:
        _id = int(input(f'{term_color.ENDC}Ingrese id de la venta a editar: 'f'{term_color.OKCYAN}'))
...
def printVenta(venta, _id):
    ...
    print(f'{term_color.HEADER}--------------------------------------------------------')
    print(f'{term_color.OKBLUE}',str(venta[0]),'\t\t|',str(venta[1]),'\t\t|',str(venta[2]),' |',str(_id))
    print(f'{term_color.HEADER}--------------------------------------------------------')
    print(f'{term_color.WARNING}CANTIDAD PRODUCTO | ID VENTA')
    print(f'{term_color.HEADER}------------------------------------')
    print(f'{term_color.OKBLUE}',str(venta[3]),'\t\t|',str(_id))
    print(f'{term_color.HEADER}------------------------------------\n'f'{term_color.ENDC}')
...
def ventaForm():
    venta = []
    venta.append(input(f'{term_color.ENDC}Ingrese código del cliente: 'f'{term_color.OKGREEN}'))
    venta.append(input(f'{term_color.ENDC}Ingrese código del vendedor: 'f'{term_color.OKGREEN}'))
    venta.append(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    venta.append(input(f'{term_color.ENDC}Cantidad de productos: 'f'{term_color.OKGREEN}'))
    return venta
